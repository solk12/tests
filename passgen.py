...
class PassGen:
    ...
    def question(self, target):
        ...
        answers['nickname'] = self.prompt('Введите {} ник: '.format(target))

        while True:
            bday = self.prompt('Введите {}\' дату рождения (день.месяц.год): '.format(target))
    ...
    def format_names(self):
        for _ in range(self.max_count_main):
            if not self.silent:
                print(f'Создается: {len(self.passwords)}')
    ...
    def generator(self, ignore_additional = True):
        ...
        self.spouse = self.question('доп.')
        print('\n')

        self.child = self.question('доп.')
        print('\n')

        self.pet = self.question('доп.')
        print('\n')

        print('Создаются пароли... ')
        self.format_names()
        if self.silent:
            print("...генерируются {} пароли".format(len(self.passwords)))
        ...
        if not ignore_additional:
            print("Генерируются пароли")
            with open(output_file, 'at') as f:
                i = 0
                while(i < self.max_count):
                    if not self.silent:
                        print('Идет запись паролей на устройство ... {}/{}'.format(i*3, self.max_count*3))
                    f.write('{}{}\n'.format(self.target['firstname'], i))
                    f.write('{}{}\n'.format(self.target['lastname'], i))
                    f.write('{}{}\n'.format(self.target['nickname'], i))
                    i += 1
            if self.silent:
                print("...записалось {} комбинаций".format(self.max_count*3))

        print('Пароли были записаны в файл: {}'.format(output_file))
        quit()
